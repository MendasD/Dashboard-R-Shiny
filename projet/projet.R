...
plats_data <- read_excel("C:/Users/DELL/Desktop/ShinyApp/test/plats.xlsx")
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Plats", tabName = "Plats", icon = icon("utensils"))
    )
  ),
  dashboardBody(
    tags$head(
    ...
    tabItems(
      tabItem(tabName = "accueil", h2("Bienvenue")),
      tabItem(tabName = "Plats",
              tabsetPanel(
                tabPanel("Informations",
                         ...
                         fluidRow(
                           column(4,
                                  selectInput("type_plat", "Type de plat :",
                                              choices = c("Tous", sort(unique(plats_data$type))),
                                              selected = "Tous")
                           ),
                           column(4,
                                  textInput("recherche_plat", "Rechercher un plat :", "")
                           ),
                           column(4,
      ...
      row <- fluidRow(
        lapply(0:3, function(j) {
          ...
          if (idx <= nrow(plats_filtrés)) {
            column(3,
                   actionButton(
                     inputId = paste0("plat_", plats_filtrés$id_plat[idx]),
